...
document.addEventListener("DOMContentLoaded", () => {
    // ... (Mantén toda tu lógica anterior de sliders y efectos aquí) ...

    // --- LÓGICA DE LIGHTBOX CORREGIDA ---
    ...
    // Función para actualizar y abrir
    function openLightbox(index, imagesArray) {
        allImages = imagesArray;
        currentImgIndex = index;
        fullImg.src = allImages[currentImgIndex].src;
        lightbox.style.display = "flex";
    }

    // Evento de clic en cualquier imagen dentro de un frame
    document.querySelectorAll(".gallery-frame img").forEach((img, index) => {
        img.style.cursor = "pointer";
        img.addEventListener("click", () => {
            // Recolectar todas las imágenes en orden del documento
            const imagesList = Array.from(document.querySelectorAll(".gallery-frame img"));
            const clickedIndex = imagesList.indexOf(img);
            openLightbox(clickedIndex, imagesList);
        });
    });

    // Función global para los botones del lightbox
    window.changeImage = function(step) {
        if (allImages.length === 0) return;
        currentImgIndex = (currentImgIndex + step + allImages.length) % allImages.length;
        fullImg.src = allImages[currentImgIndex].src;
    };

    // Cerrar
    const closeFunc = () => { lightbox.style.display = "none"; };
    closeLightbox.addEventListener("click", closeFunc);
